...
public:
    ...
    /// \param timestamp
    ///
    MaskWriteRegisterRequest(const QModbusPdu& pdu, ProtocolType protocol, int deviceId, int transactionId, const QDateTime& timestamp)
...
public:
    ...
    /// \param timestamp
    ///
    MaskWriteRegisterResponse(const QModbusPdu& pdu, ProtocolType protocol, int deviceId, int transactionId, const QDateTime& timestamp)
    ...
    ///
    bool isValid() const override {
        return ModbusMessage::isValid() && (isException() ? true : (dataSize() == 6));
    }
